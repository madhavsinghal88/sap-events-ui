...
function mapSapResult(item, index) {
  ...
  const link = normalizeLink(publicUrl);
  const rawDate = item.eventDates || item.startDate || item.pubDate || item.publishDate || '';
  return {
    id: String(index + 1),
    title: normalizeEventTitle(item.title || slugToTitle(publicUrl)),
    date: formatEventDate(rawDate, link),
...
function getLocalAppUrl() {
  ...
  const port = process.env.LOCAL_PORT || process.env.PORT || '8881';
  return `http://${host}:${port}`;
}

function getBrowserImportScript() {
  const importUrl = `${getLocalAppUrl()}/api/events/import`;
  return `(async () => {
  const PAGE_SIZE = 60;
...
module.exports = {
  ...
  mapSapResult,
  getLocalAppUrl,
  getBrowserImportScript,
  get BROWSER_IMPORT_SCRIPT() {
    return getBrowserImportScript();
  },
};
